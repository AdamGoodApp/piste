class OffPiste < ApplicationRecord
  ...
  def self.search_name_advanced(params)
    if params.key?('filter')
      ...
      OffPiste.search('*', order: { params[:field] => params[:order] }).map { |p| p }
    end
  end
  ...
  def self.search_by_greater(params)
    OffPiste.search('*', where: { params[:field] => {gt: params[:field_number].to_i} } , order: { params[:field] => params[:order] }).map { |p| p }
  end

  def self.search_by_less(params)
    OffPiste.search('*', where: { params[:field] => {lt: params[:field_number].to_i} } , order: { params[:field] => params[:order] }).map { |p| p }
  end
